...
export default function Sparkline({ points, unit, bars, format }: { points: Point[]; unit: string; bars?: boolean; format: (v: number) => string }) {
  ...
  if (bars) lo = 0
  if (hi - lo < 1e-6) { hi += 1; lo -= bars ? 0 : 1 } // flat series still needs a band to draw in
  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full touch-none select-none" role="img"
        aria-label={`Last 24 hours, ${format(lo)} to ${format(hi)} ${unit}`}
      ...
      <div className="flex justify-between text-[11px] tabular-nums text-slate-400">
        <span>min {format(lo)} · max {format(hi)} {unit}</span>

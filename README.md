...
| `POST /api/internal/ingest/stations` | Ingest from CWA. `Authorization: Bearer $INGESTION_SECRET` |

## Scheduled jobs (Supabase Cron)

`ingest-stations` every 10 min; `prune` nightly (10-minute data for 3 days, hourly for 60, then deleted — sized for the
